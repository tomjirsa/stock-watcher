...
@st.cache_data
def load_backtest_results(run_id: str) -> dict | None:
...
def get_available_backtest_runs() -> list[str]:
    if not BACKTEST_DIR.exists():
        return []
    files = sorted(BACKTEST_DIR.glob("????-??-??_??????.json"), reverse=True)
    return [f.stem for f in files]
...
def _format_run_id(run_id: str) -> str:
    ...
    if len(parts) == 2:
        t = parts[1]
        return f"{parts[0]}  {t[:2]}:{t[2:4]}:{t[4:]}"
    return run_id

def _read_backtest_config() -> dict:
    try:
...
with tab_backtest:
    ...
    selected_run = st.selectbox(
        "Backtest run",
        runs,
        format_func=_format_run_id,
    )

    bt = load_backtest_results(selected_run)
    if bt is None:
        st.warning("Could not load selected run.")
        st.stop()

    st.caption(f"Generated: {bt.get('generated', 'unknown')}  ·  Hold: {bt.get('config', {}).get('hold_days', '?')} days  ·  Threshold: {bt.get('config', {}).get('threshold', '?')}")
